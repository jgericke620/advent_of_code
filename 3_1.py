...
class Path(object):
    ...
    def calculate(self):
        ...
        for count, i in enumerate(self.directions):
            try:
                if i.startswith('D'):
                ...
                raise IndexError(f"Only at {count} out of {total} directions")
    ...
    def update_grid(self, value, down_not_up=0, right_not_left=0):
        for _ in range(value):
            ...
            self.index[0] += right_not_left
            self.index[1] += down_not_up
        

    @classmethod
    def get_distance(cls, grid1, grid2):
        
        def print_and_quit(x, y):
            print(x, y, "-->", x+y)
            quit()
